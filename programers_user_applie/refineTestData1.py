...
data_test = pd.read_csv("./datas/test_job.csv")
...
for i in range(0, len(data_test)):
    all = np.concatenate((users_one_hot[data_test["userID"][i]], jobs_one_hot[data_test["jobID"][i]], company_one_hot[data_test["jobID"][i]]), axis=0)
    X = np.append(X, np.array([all]), axis=0)

np.save("./datas/X1_Test_6000", X)
